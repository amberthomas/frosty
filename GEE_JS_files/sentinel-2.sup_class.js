var not_water_hand_select =
    ...
    ee.Geometry.MultiPolygon(
        [[[[-150.5286375827194, 62.01978884727675],
         ...
         [[[-147.56460573362529, 62.160120900029824],
           [-147.56460573362529, 62.154929690547995],
         ...
         [[[-151.1882069659498, 61.69116080889442],
           ...
           [-151.18039637329844, 61.6870088261197],
           [-151.18039637329844, 61.69116080889442]]],
         ...
         [[[-150.89153132417835, 62.3701335972062],
           [-150.89153132417835, 62.367426758072526],
...
var data_fc =  water_data.merge(not_water);

/*******************classifier****************************/

// Green/SWIR
var mndwi = s2sum.normalizedDifference(['B3', 'B11']).rename('mndwi');
var mndwiW = s2win.normalizedDifference(['B3', 'B11']).rename('mndwi');

// Green/ NIR
var mndwi2 = s2sum.normalizedDifference(['B3', 'B8']).rename('mndwi_NIR');
var mndwi2W = s2win.normalizedDifference(['B3', 'B8']).rename('mndwi_NIR');

// Change this threshold to allow more or less water in mask
var ndwiThreshold = 0.4;

/**
var watersum = mndwi.gt(ndwiThreshold);
Map.addLayer(watersum.clip(rect), {min:0, max:1, palette:['black', 'blue']}, 'water summer', true);
**/

var waterwin = mndwiW.gt(ndwiThreshold);
Map.addLayer(waterwin.clip(roi), {min:0, max:1, palette:['black', 'blue']}, 'water winter', true);


s2sum = s2sum.addBands(mndwi).addBands(mndwi2);
s2win = s2win.addBands(mndwiW).addBands(mndwi2W);


// Use these bands for classification.
var bands = ["B2","B3","B4","B5","B6","B7","B8","B11", "mndwi", "mndwi_NIR"];
// The name of the property on the points storing the class label.
var classProperty = 'landcover';


// add cols of random fps to allow us to divide pixels into train and test set
// and another to allow sampling from both winter and summer
var splittable_dataset = data_fc.randomColumn('train_test').randomColumn('sum_win');

// We want to reserve some of the data for testing, to avoid overfitting the model.
var tt_split = 0.625;  // 50 points for training another 30 for test
var trainingPartition = splittable_dataset.filter(ee.Filter.lt('train_test', tt_split));
var testingPartition = splittable_dataset.filter(ee.Filter.gte('train_test', tt_split));

var ws_split = 0.7;  // split is % of winter points
var trainPart_winter = trainingPartition.filter(ee.Filter.lt('sum_win', ws_split));
var trainPart_summer = trainingPartition.filter(ee.Filter.gte('sum_win', ws_split));
var testPart_winter = testingPartition.filter(ee.Filter.lt('sum_win', ws_split));
var testPart_summer = testingPartition.filter(ee.Filter.gte('sum_win', ws_split));

//should probably have split season first and then test train
//TODO swap

/************************get train*****************************/
var training_winter = s2win.select(bands).sampleRegions({
  collection: trainPart_winter,
  properties: [classProperty],
  scale: 20 // SWIR has 20 res, rest I think are 10
});

var training_summer = s2sum.select(bands).sampleRegions({
  collection: trainPart_summer,
  properties: [classProperty],
  scale: 20 // SWIR has 20 res, rest I think are 10
});

var training = training_winter.merge(training_summer);
/***********************get test ******************************/

var testing_winter = s2win.select(bands).sampleRegions({
  collection: testPart_winter,
  properties: [classProperty],
  scale: 20 // SWIR has 20 res, rest I think are 10
});

var testing_summer = s2sum.select(bands).sampleRegions({
  collection: testPart_summer,
  properties: [classProperty],
  scale: 20 // SWIR has 20 res, rest I think are 10
});

var testing = testing_winter.merge(testing_summer);
/***********************do classification******************************/
...
//var test = testing.classify(trainedClassifier);
...
// Classify WINTER composite
var classified = s2win.classify(trainedClassifier);
Map.addLayer(classified, {min: 0, max: 1, palette: ['blue', 'black']}, 'winter classified');

// Classify SUMMER composite
var classifiedS = s2sum.classify(trainedClassifier);
Map.addLayer(classifiedS, {min: 0, max: 1, palette: ['blue', 'black']}, 'summer classified');
